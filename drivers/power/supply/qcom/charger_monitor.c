...
/* Jie.Huang@ODM_WT.BSP.Charger.Basic.1941873, 20191014, Modify charge current max */
#define MAX_USB_ICL_CURRENT (2200000) //2000000
...
#define TEMP_CURRENT_2200_MA (MAX_USB_ICL_CURRENT)
...
struct smb_step_chg g_chg_ranges[] = {
	...
	[         BATTERY_STATUS__REMOVED] = {-3000, -3000,      0, 3980, 3680, 3990, 4000, POWER_SUPPLY_HEALTH_DEAD},
	[        BATTERY_STATUS__LOW_TEMP] = { -190,  -190,       0, 3980, 3680, 3990, 4000, POWER_SUPPLY_HEALTH_COLD},
	[       BATTERY_STATUS__COLD_TEMP] = {  -20,   -20,  350000, 3980, 3680, 3990, 4000, POWER_SUPPLY_HEALTH_COOL},
	[BATTERY_STATUS__LITTLE_COLD_TEMP] = {    0,     0,  650000, 4430, 4235, 4435, 4450, POWER_SUPPLY_HEALTH_GOOD},
	[       BATTERY_STATUS__COOL_TEMP] = {   50,    50, 1350000, 4430, 4335, 4435, 4450, POWER_SUPPLY_HEALTH_GOOD},
	[BATTERY_STATUS__LITTLE_COOL_TEMP] = {  120,   120, 2000000, 4430, 4335, 4435, 4450, POWER_SUPPLY_HEALTH_GOOD},
	[          BATTERY_STATUS__NORMAL] = {  160,   160, 2200000, 4430, 4335, 4435, 4450, POWER_SUPPLY_HEALTH_GOOD},
	[       BATTERY_STATUS__WARM_TEMP] = {  440,   440, 1350000, 4130, 4030, 4140, 4150, POWER_SUPPLY_HEALTH_WARM},
	[       BATTERY_STATUS__HIGH_TEMP] = {  530,   530,       0, 4130, 4030, 4140, 4150, POWER_SUPPLY_HEALTH_OVERHEAT},
};
...
/* Jie.Huang@ODM_WT.BSP.Charger.Basic.1941873, 20191015, Add for do ICL 15 mins later when do ICL */
static int charger_monitor_work_times = 0;
/* Jie1.Huang@ODM_WT.BSP.Charger.Basic.1941873, 20191030, Add current limited according to skin tempreture */
static int g_skin_temp_status = 0;
...
int do_charger_icl(struct smb_charger *chg)
{
	...
	if ((g_charger_para.chg_present != 1) || (chg->real_charger_type == POWER_SUPPLY_TYPE_USB)
...
static void soc_reduce_slow_when_1(struct smb_charger *chg)
{
	...
	/* Jie.Huang@ODM_WT.BSP.Charger.Basic.1941873, 20191030, Modify update ui_soc = 0 voltage */
	if (g_charger_para.batt_vol < 3400) { //3410
		reduce_count++;
...
/* Jie1.Huang@ODM_WT.BSP.Charger.Basic.1941873, 20191030, Add current limited according to skin tempreture */
void lcd_on_change_charge_current_according_skin_temp(struct smb_charger *chg)
{
	if (g_skin_temp_status_1 == 0) {
		if (chg->skin_temp > TEMP_SKIN_390) {
			g_skin_temp_status_1 = 40;
			vote(chg->usb_icl_votable, SKIN_TEMP_VOTER, true, TEMP_CURRENT_1200_MA);
		} else if (chg->skin_temp > TEMP_SKIN_370) {
	...
	if (g_skin_temp_status_1 == 36) {
		...
		if (chg->skin_temp > TEMP_SKIN_390) {
			g_skin_temp_status_1 = 40;
			vote(chg->usb_icl_votable, SKIN_TEMP_VOTER, true, TEMP_CURRENT_1200_MA);
		} else if (chg->skin_temp <= TEMP_SKIN_370 - 10) {
			g_skin_temp_status_1 = 36;
			vote(chg->usb_icl_votable, SKIN_TEMP_VOTER, true, TEMP_CURRENT_2000_MA);
		} else {
			vote(chg->usb_icl_votable, SKIN_TEMP_VOTER, true, TEMP_CURRENT_1500_MA);
		...
		if (chg->skin_temp <= TEMP_SKIN_370 - 10) {
			g_skin_temp_status_1 = 36;
			vote(chg->usb_icl_votable, SKIN_TEMP_VOTER, true, TEMP_CURRENT_2000_MA);
		} else if (chg->skin_temp <= TEMP_SKIN_390 - 10) {
			g_skin_temp_status_1 = 38;
			vote(chg->usb_icl_votable, SKIN_TEMP_VOTER, true, TEMP_CURRENT_1500_MA);
		} else {
			vote(chg->usb_icl_votable, SKIN_TEMP_VOTER, true, TEMP_CURRENT_1200_MA);
...
/* Jie1.Huang@ODM_WT.BSP.Charger.Basic.1941873, 20191030, Add current limited according to cpu tempreture */
void change_charge_current_according_cpu_temp(struct smb_charger *chg)
{
	if (g_cpu_temp_status_1 == 0) {
		if (chg->cpu_temp > TEMP_CPU_460) {
			g_cpu_temp_status_1 = 47;
		} else if (chg->cpu_temp > TEMP_CPU_430) {
	...
	if (g_cpu_temp_status_1 == 42) {
		...
		if (chg->cpu_temp > TEMP_CPU_460) {
			g_cpu_temp_status_1 = 47;
		} else if (chg->cpu_temp <= TEMP_CPU_430 - 10) {
			g_cpu_temp_status_1 = 42;
		}
	} else {
		if (chg->cpu_temp <= TEMP_CPU_430 - 10) {
			g_cpu_temp_status_1 = 42;
		} else if (chg->cpu_temp <= TEMP_CPU_460 - 10) {
			g_cpu_temp_status_1 = 45;
		}
	}
	if (g_cpu_temp_status_1 == 42)
...
/* Jie1.Huang@ODM_WT.BSP.Charger.Basic.1941873, 20191030, Add current limited according to battery tempreture */
void change_charge_current_according_battery_temp(struct smb_charger *chg)
{
	...
	if (g_batt_temp_status == 0) {
		if (g_charger_para.batt_temp > TEMP_BATT_370) {
			g_batt_temp_status = 38;
		} else if (g_charger_para.batt_temp > TEMP_BATT_350) {
	...
	if (g_batt_temp_status == 34) {
		...
		} else if (g_charger_para.batt_temp > TEMP_BATT_350 - 10) {
			batt_temp_38_flag = 0;
			batt_temp_36_flag = 0;
			batt_temp_34_flag = 0;
		} else {
		...
		if (g_charger_para.batt_temp > TEMP_BATT_370 - 10) {
			batt_temp_38_flag = 0;
			batt_temp_36_flag = 0;
			batt_temp_34_flag = 0;
		} else if (g_charger_para.batt_temp > TEMP_BATT_350 - 10) {
	...
	if (g_batt_temp_status == 38) {
		if (g_charger_para.batt_temp >= 440)
			vote(chg->usb_icl_votable, FB_BLANK_VOTER, true, min(g_charger_para.lcd_on_chg_curr_proc, 500000));
		else
			vote(chg->usb_icl_votable, FB_BLANK_VOTER, true, min(g_charger_para.lcd_on_chg_curr_proc, TEMP_CURRENT_1200_MA));
	} else if (g_batt_temp_status == 36)
		vote(chg->usb_icl_votable, FB_BLANK_VOTER, true, min(g_charger_para.lcd_on_chg_curr_proc, TEMP_CURRENT_1500_MA));
	else if (g_batt_temp_status == 34)
		vote(chg->usb_icl_votable, FB_BLANK_VOTER, true, min(g_charger_para.lcd_on_chg_curr_proc, TEMP_CURRENT_2000_MA));
	else
...
void update_charger_status(struct smb_charger *chg)
{
	...
	if (g_charger_para.lcd_is_on == 1) {
		/* Jie1.Huang@ODM_WT.BSP.Charger.Basic.1941873, 20191030, Add current limited according to skin tempreture */
		lcd_on_change_charge_current_according_skin_temp(chg);
		/* Jie1.Huang@ODM_WT.BSP.Charger.Basic.1941873, 20191030, Add current limited according to cpu tempreture */
		change_charge_current_according_cpu_temp(chg);
		/* Jie1.Huang@ODM_WT.BSP.Charger.Basic.1941873, 20191030, Add current limited according to battery tempreture */
		change_charge_current_according_battery_temp(chg);
	...
	/* Jie.Huang@ODM_WT.BSP.Charger.Basic.1941873, 20191015, Add for do ICL 15 mins later when do ICL */
	charger_monitor_work_times++;
	if (charger_monitor_work_times >= 180)
		set_icl_flags(chg, 20);

	if ((g_charger_para.chg_ranges_save != g_charger_para.chg_ranges_now) || (g_charger_para.need_do_icl != 0)) {
		do_charger_icl(chg);
		charger_monitor_work_times = 0;
	}
...
static ssize_t charging_limit_current_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
	...
	g_charger_para.lcd_on_chg_curr_proc = tmp * 1000;
	return len;
}
