...
#define BID_DESAY_SDI_H_MV	(1266)
#define BID_DESAY_SDI_L_MV	(1181)
#define BID_DESAY_SDI_OHM		(470000)
...
/* Bin2.Zhang@ODM_WT.BSP.Charger.Basic.1941873, 20190530, Add for Battery ID voltage range */
	...
	} else if ((g_batt_id_mv >= BID_DESAY_SDI_L_MV) && (g_batt_id_mv <= BID_DESAY_SDI_H_MV)) {
		profile_node = of_batterydata_get_best_profile(batt_node,
					BID_DESAY_SDI_OHM / 1000, NULL);
		devinfo_info_set("Battery", "V1.0", "SDI");
	} else {
		profile_node = of_batterydata_get_best_profile(batt_node,
					BID_DEFAULT_OHM / 1000, NULL);
		hardwareinfo_set_prop(HARDWARE_BATTERY_ID, DEFAULT_BATT_TYPE);
		devinfo_info_set("Battery", "V1.0", "UNKNOWN");
	}
#endif /* ODM_WT_EDIT */
