...
//liwei.a@oppo.com,2019.04.18, add for distinguish backlight ic, 0 for aw9962, 1 for ktd3136
static int flag = 0;
#endif /*VENDOR_EDIT*/
...
int lm3697_lcd_backlight_set_level(unsigned int bl_level)
{
	...
	int ret = 0;
	/* if aw9962 works, no need to set register*/
    if(flag == 0)
        return 0;
...
static int lm3697_chip_init(struct lm3697_bl_chip *pchip){
	...
	if (is_ktd3136 == 1) {
		...
		ret = regmap_write(pchip->regmap, 0x18, 0x15);    //Bank B Full-scale current (21.8mA)
		if (ret < 0)
			goto out;
               ...
               const struct i2c_device_id *id)
{
	...
	/*ktd3136 probe success, set flag to 1*/
    flag = 1;
